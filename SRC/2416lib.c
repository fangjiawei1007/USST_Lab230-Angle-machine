...
extern unsigned int InPulseCount[7];
//***************************[ SYSTEM ]***************************************************
void Timer3_Init(void)
{
	...
	pISR_TIMER3 = (int)Timer3_Proc;	// Timer ISR for DA
	rTCFG0 &= ~(0xff << 8); 
	rTCFG0 |= (110 << 8); 	// Dead zone=0, Prescaler0=110(0xff)   Timer 2 3 4 共用
	rTCFG1 &= ~(0xf << 12); 
	rTCFG1 |= (0x3 << 12); 	// Interrupt, Mux0=1/16
	//rTCNTB2 = 37500;    		// Timer input clock Frequency = PCLK / {prescaler value+1} / {divider value}	1 Hz
	rTCNTB3 = 37500;    		// Timer input clock Frequency = PCLK / {prescaler value+1} / {divider value}	100 Hz -- 10ms
	//rTCMPB2 = 20000;
...
void Timer3_Start(void)
{
	...
	rTCON = tmp | (2 << 16);		// update TCVNTB2, stop			
...
{
	rINTMSK1 |= BIT_TIMER3;
	rTCON &= ~(1 << 16);			// Timer2, stop						
}

char delaytime;
void __irq Timer3_Proc(void) 
{
...
///10s定时器
	...
	if( delaytime >= 1)
	{
		g_InteralMemory.Word[500] = 60 * InPulseCount[2] / Encoder_Pulse  * PI *  (double)g_InteralMemory.KeepWord[2]/10 / 100;////除以100 将毫米换算成分米 界面显示再引入一位小数点
		
		InPulseCount[2]=0;
		
		delaytime=0;//g_InteralMemory.KeepWord[169]
