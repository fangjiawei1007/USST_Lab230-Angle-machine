...
//	InPulseCount[2]++;///定时器内用于计算速度
	...
	if(Chongkou_on_off > 0)
	{
		...
		else
		{
			...
			if((InPulseCount[3] >= 0) && (InPulseCount[3] <(Inpulse_dengdai1[chongkou_current] -buchang_pulse1) ))
			{
	...
	if((Dingchang_on_off > 0) && (g_InteralMemory.KeepBit[10] == 1) && (cut_all > 0))///定长切程序KB10 冲口与定长切互联开始定长切标志位
	{
		...
		if((InPulseCount[1]>=0) && (InPulseCount[1] <Inpulse_dengdai2[dingchang_current]) )
		{
...
void qiangqie_function(void)
{
	...
	if((InPulseCount[4]>=0) && (InPulseCount[4] <20) )
